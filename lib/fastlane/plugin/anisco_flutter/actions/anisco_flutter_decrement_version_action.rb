...
module Fastlane
  module Actions
    class AniscoFlutterDecrementVersionAction < Action
      def self.run(params)
        ...
        previous_version = version.decrement
        version.update_pubspec_version(pubspec, pubspec_location)

        UI.success("Version updated: #{current_version} -> #{previous_version}")
        previous_version
      end

      def self.description
        'Decrement Flutter version in pubspec.yaml using 99-based rollover rules'
      end
